...
years = [str(year) for year in range(1920,2021)]

with open('data/coco_annotations/captions_train2014.json', 'r') as f:
    captions_data = json.load(f)
captions = sorted(list(set([ann['caption'].lower().strip() for ann in captions_data['annotations']])))
print(years)
print(len(captions))

colors = ["red", "green", "blue", "yellow", "purple", "orange", "pink", "brown", "cyan", "magenta", "lime", "maroon", "navy", "olive", "teal", "aqua", "silver", "gold"]
...
for i,caption in enumerate(captions):
    ...
    for color in colors:
        if ' '+color+' ' in caption.lower():
            #print (caption)
            captions[i] = caption.replace(color+' ','')
...
for i,caption in enumerate(captions):
    seed = 1000000+i
    print(seed)
    idx_year1 = random.randint(0,len(years)-1)
    idx_year2 = random.randint(0,len(years)-1)
    idxs = [idx_year1,idx_year2]
    gen.manual_seed(seed)
    prompt1 = 'A '+years[idx_year1]+' photo of '+caption
    prompt2 = 'A '+years[idx_year2]+' photo of '+caption
    ...
    cross_attention_kwargs = {"edit_type": "replace",
                              "n_self_replace": 0.4,
                              "n_cross_replace": {"default_": 1.0, years[idx_year2]: 0.4},
                              }
...
print(f"Num images: {len(image['images'])}")
for i,img in enumerate(image['images']):
    img = img.save('p2p_demo_reweight_'+str(i)+'.jpg')
...
image = pipe(prompts, cross_attention_kwargs=cross_attention_kwargs, generator=g_cpu)
for i,img in enumerate(image['images']):
    img = img.save('p2p_demo_refine_'+str(i)+'.jpg')
"""
